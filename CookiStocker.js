...
function stockerTimeBeautifier(duration) {
    ...
    if (seconds && (minutes || hours || days) && !stockerForceLoopUpdates) seconds = 0; // Don't display
    ...
    var strHours = hours
        ? hours +
          " hour" +
          (hours != 1 ? "s" : "") +
          (minutes && seconds ? ", " : (minutes ? !seconds : seconds) ? " and " : "")
        : "";
    var strDays = days
        ? days +
          " day" +
                ...
                var stockerLoop = setInterval(function () {
                    ...
                    for (let i = 0; i < market.length; i++) {
                        ...
                        if (
                            ...
                            // buying
                            let mode =
                                lmd != md ? "is no longer in " + modeDecoder[lmd] + " mode" : "is ";
                            ...
                            if (
                                !Game.Objects["Bank"].minigame.sellGood(i, stockList.goods[i].stock)
                            ) {
                    ...
                    for (let i = 0; i < market.length; i++) {
                        ...
                        if (stockList.goods[i].profit > 0) stockList.sessionProfitableStocks++;
                    ...
                    var stockerReportInterval = setInterval(function () {
                        ...
                        if (
                            ...
                            stockList.sessionStart +=
                                now - stockList.sessionLastTime - stockerActivityReportFrequency;
                        }
                            ...
                            if (stockList.sessionPurchases + stockList.sessionSales == 0) {
                                ...
                                Game.Notify(
                                    ...
                                    `This session has been running for ${stockerTimeBeautifier(
                                        ...
                                        stockList.sessionPurchases,
                                        0
                                    )} purchases and ${Beautify(stockList.sessionSales, 0)} sales.`,
                                    [1, 33],
                                    stockerFastNotifications
                                );
                            }
                        if (stockerConsoleAnnouncements) {
                            ...
                            for (j = 0; j < 6; j++)
                                for (k = 0; k < 6; k++) totalProfits += stockerModeProfits[j][k][0];
                            for (j = 0; j < 6; j++)
                                for (k = 0; k < 6; k++) {
